...
1. Build JSONL shards with `coscope.main`
...
Run the full MuSiQue dev CoT job with DashScope/Qwen and export a shareable bundle:
    ...
    python -m coscope.scripts.run_trace_job \
        --dataset musique \
        --split dev \
        ...
        --llm dashscope \
        --llm-model qwen-plus \
        --processed-dir coscope/data/processed_qwen_full \
        --export-root coscope/data/exports \
...
from coscope.main import main as run_main
from coscope.scripts.export_trace_bundle import export_trace_bundle
...
def _build_main_argv(args: argparse.Namespace) -> List[str]:
    ...
    if args.id_manifest:
        argv.extend(["--id-manifest", args.id_manifest])
    if args.strict_id_match:
        argv.append("--strict-id-match")
    if args.write_id_manifest:
        argv.extend(["--write-id-manifest", args.write_id_manifest])
    if args.audit_after_build:
        argv.append("--audit-after-build")
...
def main() -> int:
    ...
    parser.add_argument("--export-root", default="coscope/data/exports")
    ...
    parser.add_argument("--id-manifest", default=None)
    parser.add_argument("--strict-id-match", action="store_true")
    parser.add_argument("--write-id-manifest", default=None)
    parser.add_argument("--resume-existing", action="store_true")
    parser.add_argument("--audit-after-build", action="store_true", default=True)
    parser.add_argument("--no-audit-after-build", dest="audit_after_build", action="store_false")
