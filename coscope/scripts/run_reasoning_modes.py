"""Run live CoT and retrieval-aware ToT comparisons on fixed samples."""
...
from coscope.evaluation.mode_experiment import run_reasoning_mode_comparison
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="new_results/reasoning_mode_comparison.json",
    )
    ...
    report = run_reasoning_mode_comparison(
        examples,
        ...
        json.dumps(
            {
                ...
                "benchmarks": {
                    benchmark: {
                        mode: result["aggregate"]
                        for mode, result in modes.items()
                    }
                    for benchmark, modes in report["benchmarks"].items()
                },
            },
            ensure_ascii=False,
            indent=2,
