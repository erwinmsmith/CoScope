#!/usr/bin/env bash
# Re-run eval_jsonl at k=5 and k=20 for every existing Stage A shard set
# across (rpt x dataset). Reuses the workspace embedding cache, so query
# embeddings are mostly cache hits and no LLM calls are made.
#
...
VARIANTS_FULL="a1 a2 a3 a4 a4_nofb a4_norerank"

run_eval() {
  local shard_dir="$1"      # directory holding s*.jsonl
  local out_prefix="$2"     # e.g. eval_v9_qwen_full
  local variants="$3"
  local k="$4"
...
# Stage A (A1-A4 variants) at k=5 and k=20, RPT in {got, tot}.
