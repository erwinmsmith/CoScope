...
class Serializer:
    ...
    def episode_to_dict(self, episode: Episode) -> Dict[str, Any]:
        ...
        # Enrich got_graph.nodes with the artifact ids and full content each node produced, so
        # a consumer can read "what this node output" without scanning
        # memory_entries. See Schema.md §7.
        got_graph_dict = episode.got_graph.to_dict()
        by_node: Dict[str, Dict[str, Any]] = {}
        
        # Create memory lookup for faster access
        memory_lookup = {m.memory_id: m for m in episode.memory_entries}
        
        for m in episode.memory_entries:
            md = m.metadata or {}
            slot = md.get("slot")
            node_id = md.get("source_node_id")
            ...
            bucket["produced_artifact_ids"].append(m.memory_id)
            bucket["produced_by_slot"][slot] = m.memory_id
            bucket["produced_artifacts"].append(self._memory_entry_to_dict(m))
        
        for n in got_graph_dict.get("nodes", []):
            info = by_node.get(n.get("node_id"), {
                "produced_artifact_ids": [], 
                "produced_by_slot": {},
                "produced_artifacts": []
            })
            n["produced_artifact_ids"] = list(info["produced_artifact_ids"])
            n["produced_by_slot"] = dict(info["produced_by_slot"])
            n["produced_artifacts"] = list(info["produced_artifacts"])
