...
- :mod:`dataio.loaders` -- raw-dataset loaders (MuSiQue, HotpotQA,
  2WikiMultihopQA, MATH, GSM8K, ...). They convert third-party file formats
  into CoScope's internal raw-item dict.
- :mod:`dataio.serializer` -- :class:`Serializer` for the JSONL episode
  format (round-trips :class:`~core.types.Episode`).
- :mod:`dataio.stats_reporter` -- subset/coverage reporting over
  serialized episode shards.
- :mod:`dataio.validator` -- batch-level JSONL validation.
